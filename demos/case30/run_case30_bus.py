"""
Run TSUM on IEEE 30-bus DC-OPF blackout model (branches + buses).

Components: 71 total (30 buses + 41 branches)
  - 6 generator buses: 4-state (0=removed, 1=40%cap, 2=80%cap, 3=full)
  - 24 ordinary buses: 2-state (0=failed, 1=operational)
  - 41 branches: 2-state (0=failed, 1=operational)
System function: DC-OPF, blackout_threshold=40.2% (Scenario 1 from paper)
Reference: Chan et al. (2024), Table 2: p_f ~ 1.0e-4

Usage:
    python run_case30_bus.py
    python run_case30_bus.py --unk-prob-thres 1e-4
    python run_case30_bus.py --devices cuda:0 cuda:1
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="TSUM on IEEE 30-bus DC-OPF")
...
def main():
    ...
    print("=" * 60)
    print("TSUM on IEEE 30-bus DC-OPF (branches + buses)")
    print("=" * 60)

    # ---------------------------------------------------------------
    # 1. Load input data
    # ---------------------------------------------------------------
    data_dir = HERE / "case30_tsum_bus"
    ...
    print(f"  Threshold:   40.2% blackout (Scenario 1)")
    ...
    case_path = str(HERE / "case30.m")
    sfun = make_dcopt_sfun(
        case_path=case_path,
        blackout_threshold=40.2,
        alpha=2.0,
    )
